...
Installs the latest published SmileMenu release for:
...
echo -e "${BLUE}SmileMenu installer${NC}"
echo
...
case "$package_kind" in
    arch)
        asset_regex='^smilemenu-.+-[0-9]+-x86_64\.pkg\.tar\.zst$'
        ;;
    debian)
        asset_regex='^smilemenu_.+_amd64\.deb$'
        ;;
    fedora)
        asset_regex='^smilemenu-.+\.x86_64\.rpm$'
        ;;
esac
...
echo
echo -e "${GREEN}✓ SmileMenu ${release_tag} installed successfully!${NC}"
echo
echo "Run it with:"
echo "  smilemenu"
